import { wrapCommandWithSandboxLinux } from '@/os/linux/sandbox/linux-sandbox';
import { wrapCommandWithSandboxMacOS } from '@/os/macos/sandbox/macos-sandbox';
import { macGetMandatoryDenyPatterns } from '@/os/macos/sandbox/macos-sandbox-profile';
...
describe('Mandatory Deny Paths - Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    mkdirSync(join(TEST_DIR, '.vsbx', 'commands'), { recursive: true });
    mkdirSync(join(TEST_DIR, '.vsbx', 'agents'), { recursive: true });
    writeFileSync(
      join(TEST_DIR, '.vsbx', 'commands', 'test.md'),
      ORIGINAL_CONTENT
    );
    writeFileSync(
      join(TEST_DIR, '.vsbx', 'agents', 'test-agent.md'),
      ORIGINAL_CONTENT
    );
    ...
    writeFileSync(
      join(TEST_DIR, '.vsbx', 'some-other-file.txt'),
      ORIGINAL_CONTENT
    );
  });
  ...
  describe('Dangerous directories should be blocked', () => {
    ...
    it('blocks writes to .vsbx/commands/', async () => {
      if (skipIfUnsupportedPlatform()) return;

      const result = await runSandboxedWrite(
        '.vsbx/commands/test.md',
        MODIFIED_CONTENT
      );

      expect(result.success).toBe(false);
      expect(readFileSync('.vsbx/commands/test.md', 'utf8')).toBe(
        ORIGINAL_CONTENT
      );
    });

    it('blocks writes to .vsbx/agents/', async () => {
      if (skipIfUnsupportedPlatform()) return;

      const result = await runSandboxedWrite(
        '.vsbx/agents/test-agent.md',
        MODIFIED_CONTENT
      );

      expect(result.success).toBe(false);
      expect(readFileSync('.vsbx/agents/test-agent.md', 'utf8')).toBe(
        ORIGINAL_CONTENT
      );
    });
  ...
  describe('Safe files should still be writable', () => {
    ...
    it('allows writes to .vsbx/ files outside commands/agents', async () => {
      if (skipIfUnsupportedPlatform()) return;

      const result = await runSandboxedWrite(
        '.vsbx/some-other-file.txt',
        MODIFIED_CONTENT
      );

      expect(result.success).toBe(true);
      expect(readFileSync('.vsbx/some-other-file.txt', 'utf8').trim()).toBe(
        MODIFIED_CONTENT
      );
    });
  });
